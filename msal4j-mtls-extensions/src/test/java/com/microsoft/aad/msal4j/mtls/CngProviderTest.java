...
class CngProviderTest {
    ...
    @Test
    void provider_registersSha1WithRsa() {
        Provider p = new CngProvider();
        assertNotNull(p.getService("Signature", "SHA1withRSA"),
                "CNG provider must advertise SHA1withRSA (ADFS/DSTS compat)");
    }
    ...
    void provider_registersRsaSsaPss() {
        ...
        assertNotNull(p.getService("Signature", "RSASSA-PSS"),
                "CNG provider must advertise RSASSA-PSS (used by TLS 1.3 client cert verify)");
    }
