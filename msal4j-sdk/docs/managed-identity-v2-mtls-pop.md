...
If ESTS rejects the current binding with `invalid_client`, or the local TLS
exchange fails, MSAL invalidates the provider's current generation, mints a
replacement, and retries the token leg once. The retry is deliberately bounded;
other service errors are returned without reminting.
