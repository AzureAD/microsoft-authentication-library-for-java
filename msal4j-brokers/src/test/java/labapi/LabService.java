        ...
        return labApp
                .acquireToken(
                        ClientCredentialParameters
                                .builder(Collections.singleton(LabConstants.MSIDLAB_DEFAULT_SCOPE))
                                .build())
                .get()
                .accessToken();
    }
    ...
    User getUser(UserQueryParameters query) {
        try {
            ...
            if (query.parameters.containsKey(UserQueryParameters.FEDERATION_PROVIDER)) {
                user.setFederationProvider(
                        query.parameters.get(UserQueryParameters.FEDERATION_PROVIDER));
            } else {
                user.setFederationProvider(LabConstants.FEDERATION_PROVIDER_NONE);
            }
            return user;
        } catch (Exception ex) {
